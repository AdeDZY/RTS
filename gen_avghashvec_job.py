...
if __name__ == '__main__':
    ...
    for y in range(12, 16):
        ...
        out_dirs += ["output/avgvec/{0}{1:02d}s".format(y, i) for i in range(1, 13)]
        dirs += ["data/{0}{1:02d}s_extid".format(y, i) for i in range(1, 13)]
    vec_dirs += ["output/vectors_160{0}s".format(i) for i in range(1, 6)]
    out_dirs += ["output/avgvec/160{0}s".format(i) for i in range(1, 6)]
    dirs += ["data/160{0}s_extid".format(i) for i in range(1, 6)]

    job_file = open("get_avghashvec.job", 'w')

    for i in range(len(dirs)):
        execuatable = "./get_avg_hashvec.py"
        arguments = "{0} {1} {2}".format(dirs[i], vec_dirs[i], out_dirs[i])
